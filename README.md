...
При первом запуске YOLOv8 автоматически загрузит нужную модель из интернета.
...
2. Настройте параметры детекции (или оставьте значения по умолчанию)
...
2. Укажите индекс камеры (обычно 0 для основной камеры)
...
- Image Size - размер входного изображения, больше = точнее, но медленнее (по умолчанию 640)
...
**Фильтрация классов:**
Включите "Enable class filtering" чтобы выбрать только нужные классы объектов (например, только люди или только машины).
